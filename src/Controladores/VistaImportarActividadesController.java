...
public class VistaImportarActividadesController implements Initializable {
    ...
    private void importarActividad(ActionEvent event) {
        ...
        btnImportar.setDisable(true);
        // Abre actividad al inserir 
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/Vistas/VisualizarActividad.fxml"));
            Node mapaNode = loader.load(); 
            
            VisualizarActividadController tuControlador = loader.getController();
            tuControlador.setActivity(actividadImportada);
            
            Node botonPulsado = (Node) event.getSource();
            StackPane contentArea = (StackPane) botonPulsado.getScene().lookup("#contentArea");
            
            if (contentArea != null) {
                contentArea.getChildren().clear();
                contentArea.getChildren().add(mapaNode);
            }
        } catch (Exception e) {
